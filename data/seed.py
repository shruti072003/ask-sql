...
def load_dim_geography(geo_df: pd.DataFrame, engine) -> None:
    ...
    # Normalize before deduplicating to prevent case-sensitivity duplicate errors
    geo_df = geo_df.copy()
    geo_df["zip_code_clean"] = geo_df["geolocation_zip_code_prefix"].astype(str).str.zfill(5)
    geo_df["city_clean"] = geo_df["geolocation_city"].fillna("").astype(str).str.strip().str.title()
    
    geo_dedup = geo_df.drop_duplicates(subset=["zip_code_clean", "city_clean"])
    records = [
        {
            "zip_code_prefix": row["zip_code_clean"],
            "city": row["city_clean"] if row["city_clean"] else None,
...
def load_fact_orders(
    ...
    Aggregates order items by order_id to sum price and freight to match the fact_orders.order_id PK.
    """
    logger.info("Joining orders + aggregated order_items for fact_orders...")
    # Map customer_id → customer_unique_id
    cust_map = customers_df.set_index("customer_id")["customer_unique_id"].to_dict()

    # Aggregate order items to the order level (sum price/freight, pick first product/seller as representative)
    items_grouped = items_df.groupby("order_id").agg({
        "price": "sum",
        "freight_value": "sum",
        "product_id": "first",
        "seller_id": "first"
    ...
    merged = pd.merge(orders_df, items_grouped, on="order_id", how="left")
    logger.info("Merged orders+items: %d unique orders", len(merged))
...
def load_dim_reviews(reviews_df: pd.DataFrame, engine) -> None:
    ...
    # Deduplicate by both review_id and order_id to respect PK and UNIQUE constraints
    reviews_dedup = reviews_df.drop_duplicates(subset=["review_id"]).drop_duplicates(subset=["order_id"])
    records = [
        {
