...
Your task: Given a natural-language question, write a single, correct SQL query.
...
8. Never generate INSERT, UPDATE, DELETE, or DROP statements unless the question explicitly asks to modify, insert, delete, or drop data.
...
def _execute_sql(sql: str, allow_write: bool = False) -> list[dict[str, Any]]:
    """Run a SQL query and return results as a list of dicts.

    Only single-statement SELECT or WITH (CTE) queries are permitted by default.
    If allow_write is True, we allow modifying queries (like INSERT, UPDATE, DELETE) to run.
...
async def run_query(question: str) -> dict[str, Any]:
    ...
    try:
        ...
        results = await asyncio.to_thread(_execute_sql, generated_sql, False)
        latency_ms = int((time.monotonic() - start_time) * 1000)
